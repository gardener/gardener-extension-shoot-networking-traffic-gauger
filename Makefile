...
REGISTRY                    := europe-docker.pkg.dev/gardener-project/gardener
...
# Override golangci-lint install: Gardener's tools.mk tries to cd into a logcheck
# directory that doesn't exist in the module cache. This extension doesn't use logcheck.
...
generate: $(CONTROLLER_GEN) $(GEN_CRD_API_REFERENCE_DOCS) $(HELM) $(MOCKGEN) $(YQ)
	@REPO_ROOT=$(REPO_ROOT) GARDENER_HACK_DIR=$(GARDENER_HACK_DIR) bash $(GARDENER_HACK_DIR)/generate-sequential.sh ./charts/... ./cmd/... ./pkg/... ./test/...
	$(MAKE) format
	$(MAKE) format
