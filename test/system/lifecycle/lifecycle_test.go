...
/**
	Overview
		- Tests the lifecycle controller for the shoot-networking-traffic-gauger extension.
	Prerequisites
		- A Shoot exists and the shoot-networking-traffic-gauger extension is available for the seed cluster.
**/
...
var _ = Describe("Shoot networking traffic gauger testing", func() {
	...
	f.Serial().Beta().CIt("Should perform the common case scenario without any errors", func(ctx context.Context) {
		err := f.UpdateShoot(ctx, ensureShootNetworkingTrafficGaugerIsEnabled)
		...
		// Ensure that the networking traffic gauger is disabled in order to verify the deletion process
		err = f.UpdateShoot(ctx, ensureShootNetworkingTrafficGaugerIsDisabled)
...
func ensureShootNetworkingTrafficGaugerIsEnabled(shoot *gardencorev1beta1.Shoot) error {
...
func ensureShootNetworkingTrafficGaugerIsDisabled(shoot *gardencorev1beta1.Shoot) error {
	for i, e := range shoot.Spec.Extensions {
		if e.Type == constants.ExtensionType {
